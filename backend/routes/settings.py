...
async def get_settings():
    ...
    db = await get_db()
    try:
        cursor = await db.execute("SELECT key, value FROM settings WHERE key IN ('auto_slide_gallery', 'theme')")
        ...
        return SettingsResponse(auto_slide_gallery=auto_slide_gallery, theme=theme)
...
async def update_settings(settings: SettingsUpdate):
    ...
    try:
        ...
        return SettingsResponse(auto_slide_gallery=settings.auto_slide_gallery, theme=settings.theme)
